...
.directive('treeTableChart', [function () {
      return {
        restrict: 'EA',
        scope: {
          data: '=',
          ...
          scope.$watch( 'data', function ( data ) {
            if ( data ) {
              ...
              nv.addGraph(function () {
                ...
                .tableClass('table table-striped table-bordered') //for bootstrap styling
                .columns(scope.columnsdata);
				d3.select("#"+scope.id).datum(data).call(chart);
				return chart;
				}
              );
            }
        ...
        link: function ( scope, element, attrs ) {
          scope.$watch( 'data', function ( data ) {
            if ( data ) {
							...
							var s = new sigma({ graph: data,container: scope.id});
...
.directive('jvectorMap', [function () {
      return {
        ...
        link: function ( scope, element, attrs ) {
		 $('#'+scope.id).vectorMap({map:scope.data});
      ...
      return {
        ...
        controller: ['$scope','$element','$attrs',function ( $scope, $element, $attrs ) {
			$scope.arr = {
                ...
                url: $scope.url,
                cache: $scope.cache,
                striped: $scope.striped,
                pagination: $scope.pagination,
                pageSize: $scope.pageSize,
                pageList: $scope.pageList,
                search: $scope.search,
                showColumns: $scope.showColumns,
                showRefresh: $scope.showRefresh,
                minimumCountColumns: $scope.minimumCountColumns,
                clickToSelect: $scope.clickToSelect,
                columns: $scope.columns
				};
		
			$scope.writebootstraptbale = function (){
				$('#'+$scope.id).bootstrapTable($scope.arr);
			};
          }
        ],
        link: function ( scope, element, attrs ) {
				scope.writebootstraptbale();
        }
      };
    }
