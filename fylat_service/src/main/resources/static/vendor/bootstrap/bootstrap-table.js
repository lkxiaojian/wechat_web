...
!function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            style += sprintf('width: %s%s; ', column.checkbox || column.radio ? 10 : width, unitWidth);
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if(!this.options.hidePaginationInfo){
            html.push(
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions),this.options.jsonpOption, {
    ...
    BootstrapTable.prototype.load = function (data) {
         ...
         * 默认传进来的内容是需要是如下结果
         * {rows:[{},{}....]}
         * 如果要指定传进来的对象key不是rows需要指定resultTag表属性
         * 如果传进来的就是一个列表对象指定resultTag="tableList"
        ...
        if (this.options.sidePagination === 'server') {
            ...
            if(this.options.resultTag){
                if(this.options.resultTag.indexOf(".")!=-1){
                    var valarray = this.options.resultTag.split(".");
                    var retval=data[valarray[0]];
                    for(var i=1; i<valarray.length;i++){
                        retval = retval[valarray[i]];
                    }
                    data.rows = retval;
                }else if("tableList"==this.options.resultTag){
                    //tableList表示进来的对象就一个列表数组
                    if(this.options.jsonpOption && this.options.jsonpOption.loadLastRow){
                        if(data.length<=5){
                            data.rows = data;
                        }else{
                            data.rows = [data[data.length-1],data[data.length-2],data[data.length-3],data[data.length-4],data[data.length-5]];
                        }
                    }else{
                        data.rows = data;
                    }

                }else if(this.options.resultTag.indexOf("[]")!=-1){
                    var valarray = this.options.resultTag.split(",");
                    var index = valarray[1];
                    var attrType =  valarray[2];
                    data.rows = data[index][attrType];
                }else{
                    data.rows = data[this.options.resultTag];
                }
            }
            data = data.rows;
