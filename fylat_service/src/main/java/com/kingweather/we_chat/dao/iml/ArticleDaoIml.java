...
//            Map map1 = mapList.get(0);
        ...
        int paperNum = jdbcTemplate.queryForObject("SELECT  COUNT(article_id) FROM article b,article_type a WHERE  a.article_type_id=b.article_type_id AND b.del_type !=1 AND a.del_type !=1 AND b.article_type_id = ? AND  b.state=1  ", Integer.class, new Object[]{articleId});
...
//        }
 ...
 /*       //关注的类型sql
        ...
        String nogzSeachSql = "SELECT article_type_id,article_type_name,article_type_keyword ,create_time,iamge_icon,iamge_back ,2 as type_id from zz_wechat.article_type WHERE del_type !=1 and  parentid !=100 AND  parentid !=-1 and issue =1 AND (binary article_type_name LIKE '%" +
                message +
