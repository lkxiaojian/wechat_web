...
(function ($) {
    ...
    var WorldMap = function (params) {
        ...
        if (params.enableLegend) {
            var ftg=$(document.createDocumentFragment());
            for (i = 0; i < params.scaleColors.length; i++) {
                var color=params.scaleColors[i];
                if($.type(color)=='array'){
                    color='rgb('+color.join(',')+')'
                }
                $('<div/>').addClass('jqvmap-legend-tick')
                    .addClass('_jlt_float')
                    .css('background', color)
                    .appendTo(ftg);
            }
            var box=$('<div class="jqvmap-legend-box _jlb_clear"><div class="_jlt_float">低</div></div>')
                .css({
                    //'height':params.scaleColors.length*25
                }).append(ftg,'<div class="_jlt_float">高</div>');
            params.container.append(box)
        }
