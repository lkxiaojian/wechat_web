!function () {
    ...
    function getPercent(d) {
        if (d.value !== -1 || !isNaN(d.data.orgvalue)) {
            var str = d.endAngle - d.startAngle > 0.01 ? Math.round(1000 * (d.endAngle - d.startAngle) / (Math.PI * 2)) / 10 + '%' : '';
            return str;
        }
    }
    ...
    Donut3D.draw = function (id, data, x /*center x*/, y/*center y*/,
        ...
        if(data.length==1&&data[0].color=="#DEDEDE"){
            if(data[0].color=="#DEDEDE"){
                slices.select(".percent").remove();
            }
        }else{
            if(data[1].color=="#DEDEDE"){
                slices.selectAll(".percent")[0][1].remove();
            }
        }
    }
